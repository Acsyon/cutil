...
{
    if (needed <= ald->arr->capacity) {
        return CUTIL_STATUS_SUCCESS;
    }
    const size_t new_cap = _arraylist_next_capacity(ald->arr->capacity, needed);
...
_cutil_ArrayListIter_remove(void *data)
{
    ...
    const int rc = _cutil_ArrayList_remove_mult(ald, iter->idx, 1UL);
