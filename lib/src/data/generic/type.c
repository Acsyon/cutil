...
)
{
    ...
    CUTIL_RETURN_IF_VAL(num, 0UL);
    if (callback == NULL) {
        cutil_log_debug(
          "GenericType '%s': NULL callback, skipping %zu element(s)",
          type->name, num
        );
        return;
    }
...
)
{
    ...
    if (old_num > new_num) {
        ...
        cutil_GenericType_apply_clear_mult(type, p, diff);
    }
    ...
    if (data != NULL && new_num > old_num) {
        ...
        cutil_GenericType_apply_init_mult(type, p, diff);
    }
    return data;
}
