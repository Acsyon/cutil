...
/**
 * Returns a newly heap-allocated read-only iterator over `arr`. The iterator
 * traverses elements in index order from 0 to capacity - 1. The returned
 * iterator must be free'd.
 *
 * @param[in] arr cutil_Array to iterate over
 *
 * @return newly malloc'd cutil_ConstIterator, or NULL if `arr` is NULL
 */
cutil_ConstIterator *
cutil_Array_get_const_iterator(const cutil_Array *arr);

/**
 * Returns a newly heap-allocated read-write iterator over `arr`. The iterator
 * traverses elements in index order and supports `set`. The `remove` operation
 * is not supported (vtable slot is NULL). The returned iterator must be free'd.
 *
 * @param[in] arr cutil_Array to iterate over
 *
 * @return newly malloc'd cutil_Iterator, or NULL if `arr` is NULL
 */
cutil_Iterator *
cutil_Array_get_iterator(cutil_Array *arr);

/**
 * 'cutil_ConstIteratorType' for an array iterator (read-only).
 */
extern const cutil_ConstIteratorType *const CUTIL_CONST_ITERATOR_TYPE_ARRAY;

/**
 * 'cutil_IteratorType' for an array iterator (read-write).
 */
extern const cutil_IteratorType *const CUTIL_ITERATOR_TYPE_ARRAY;
...
extern const cutil_GenericType *const CUTIL_GENERIC_TYPE_ARRAY_HASHT;
