...
#ifndef CUTIL_CORE_IO_LOG_H_INCLUDED
#define CUTIL_CORE_IO_LOG_H_INCLUDED
 ...
 *       - Single stdout output handler
 */
cutil_Logger *
cutil_Logger_create_default(void);

/**
 * Creates a new logger instance with stdout output handler and specified
...
/**
 * Creates a new logger instance with `stream` output handler and specified
 * minimum log level `level`.
 *
 * @param[in] stream output stream of handler
 * @param[in] level minimum log level threshold for the new logger
 *
 * @return pointer to newly allocated logger
 */
...
#endif /* CUTIL_CORE_IO_LOG_H_INCLUDED */
