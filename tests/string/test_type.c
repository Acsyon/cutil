...
_should_respectMaxlen_when_createdFromNstring(void)
{
    ...
    /* Act */
    cutil_String *const s = cutil_String_from_nstring(input, 3);
...
_should_respectMaxlen_when_viewCreatedFromNstring(void)
{
    ...
    /* Act */
    cutil_StringView *const sv = cutil_StringView_from_nstring(input, 3);

    /* Assert */
    TEST_ASSERT_EQUAL_PTR(input, sv->str);
    TEST_ASSERT_EQUAL_size_t(3, sv->length);
...
{
    /* Arrange: from_nstring("hello", 3) should serialize as "hel" */
    cutil_StringView *const sv = cutil_StringView_from_nstring("hello", 3);
    char buf[16];
