...
_should_callInitAppropriateCount_when_applyInit(void)
{
    ...
    /* Act */
    const size_t num = 32;
    cutil_GenericType_apply_init(&type, ARRAY, num);
...
_should_callClearAppropriateCount_when_applyClear(void)
{
    ...
    /* Act */
    cutil_GenericType_apply_clear(&type, ARRAY, num);
...
_should_callCopyAppropriateCount_when_applyCopy(void)
{
    ...
    /* Act */
    cutil_GenericType_apply_copy(&type, DST, SRC, num);
...
_should_notCallFuncs_when_functionsAreNull(void)
{
    ...
    /* Act */
    cutil_GenericType_apply_init(&type, DST, 10);
    cutil_GenericType_apply_clear(&type, DST, 10);
    cutil_GenericType_apply_copy(&type, DST, SRC, 10);
