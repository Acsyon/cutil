...
#define TEST_HASH_EQUAL(TYPE, ID, VALUE, EXPECTED)                             \
    do {                                                                       \
        ...
        /* Act */                                                              \
        const cutil_hash_t hash1 = cutil_hash_##ID(val);                       \
        const cutil_hash_t hash2 = cutil_hashfunc_##ID(&val);                  \
        /* Assert */                                                           \
...
_should_returnValue_when_hashIntegers(void)
{
    ...
    TEST_HASH_EQUAL(int, int, -42, (cutil_hash_t) -42);
    ...
    TEST_HASH_EQUAL(int8_t, i8, -128, (cutil_hash_t) -128);
    TEST_HASH_EQUAL(int16_t, i16, -32768, (cutil_hash_t) -32768);
    TEST_HASH_EQUAL(int32_t, i32, -2147483648, (cutil_hash_t) -2147483648);
    TEST_HASH_EQUAL(
      int64_t, i64, -9223372036854775807LL,
      (cutil_hash_t) -9223372036854775807LL
    );
    ...
    TEST_HASH_EQUAL(
      size_t, size_t, 18446744073709551615ULL, 18446744073709551615ULL
    );
}
...
_should_combineHashesCorrectly_when_varyInput(void)
{
    ...
    const cutil_hash_t hash = 67890;

    /* Act */
    const cutil_hash_t combined1 = cutil_hash_combine(seed, hash);
    const cutil_hash_t combined2 = cutil_hash_combine(hash, seed);
    const cutil_hash_t combined3 = cutil_hash_combine(seed + 1, hash);
    const cutil_hash_t combined4 = cutil_hash_combine(seed, hash + 1);
...
_should_hashBytesCorrectly_when_provideString(void)
{
    ...
    const cutil_hash_t seed = 42;

    /* Act */
    const cutil_hash_t hash1 = cutil_hash_bytes(data1, len1, seed);
    const cutil_hash_t hash2 = cutil_hash_bytes(data2, len2, seed);
    const cutil_hash_t hash3 = cutil_hash_bytes(data1, len1, seed + 1);
...
_should_hashStringCorrectly_when_provideEmptyAndNonEmpty(void)
{
    ...
    const cutil_hash_t empty_hash = 5381; /* DJB33 initial value */

    /* Act */
    const cutil_hash_t hash1 = cutil_hash_str(str);
    const cutil_hash_t hash2 = cutil_hashfunc_str(str);
    const cutil_hash_t hash3 = cutil_hash_str("hello world!");
    const cutil_hash_t hash4 = cutil_hash_str("");
